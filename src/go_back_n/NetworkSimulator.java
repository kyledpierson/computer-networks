...
public abstract class NetworkSimulator {
	...
	public NetworkSimulator(int numMessages, double loss, double corrupt, double avgDelay, int trace, long seed) {
	...
	public void runSimulator() {
		...
		while (true) {
			...
			if (nSim >= maxMessages) {
				// Don't break, let the buffer get sent
				// break;
			}
			...
			case FROMLAYER5:
				...
				// Prevents the loop from generating new messages indefinitely
				if (nSim >= maxMessages)
					continue;
